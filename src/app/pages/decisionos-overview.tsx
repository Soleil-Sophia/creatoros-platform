    ...
    <main style={{ minHeight: '100vh', background: '#0E0F14', color: '#F4F3F8', padding: 32 }}>
      <div style={{ maxWidth: 1220, margin: '0 auto' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', gap: 24, alignItems: 'flex-start', marginBottom: 28 }}>
          <div>
            ...
            <p style={{ color: '#9296A8', margin: 0, maxWidth: 790, lineHeight: 1.65 }}>
              DecisionOS coordinates the platform-wide decision lifecycle. It does not own the specialist logic of BrandOS, CreatorOS, or IntelligenceOS and never replaces explicit human approval.
            </p>
          </div>
          <a href="/platform/decisions/review" style={{ borderRadius: 11, border: '1px solid rgba(218,191,255,0.25)', background: 'rgba(218,191,255,0.09)', color: '#E5D2FF', padding: '11px 14px', fontWeight: 750, textDecoration: 'none', whiteSpace: 'nowrap' }}>
            Open Review Queue
          </a>
        </header>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: 12, marginBottom: 20 }}>
          ...
          <MetricCard label="Approved, not canonical" value={approved.length} hint="Accepted changes waiting for explicit canonical apply." href="/platform/decisions/apply/brandos" />
          <MetricCard label="Canonical, not implemented" value={canonical.length} hint="Approved rules that still need operational implementation." href="/platform/decisions/outcomes" />
          <MetricCard label="Implemented, not observed" value={implemented.length} hint="Completed changes still waiting for outcome evidence." href="/platform/decisions/outcomes" />
          <MetricCard label="Observed" value={observed.length} hint="Completed learning cycles available to IntelligenceOS." href="/platform/intelligence/learning" />
        </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.25fr) minmax(320px, 0.75fr)', gap: 18, alignItems: 'start' }}>
          <section style={{ ...panelStyle, padding: 22 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
              <div>
                <div style={{ color: '#858999', fontSize: 10, fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Next operational surfaces</div>
                <h2 style={{ margin: '7px 0 0', fontSize: 20 }}>Decision lifecycle</h2>
              </div>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              ...
              <WorkflowLink title="Canonical Apply" description="Compare approved BrandOS changes before creating a new canonical revision." count={approved.length} href="/platform/decisions/apply/brandos" />
              <WorkflowLink title="Implementation & Observation" description="Record what was implemented and what real-world outcome followed." count={canonical.length + implemented.length} href="/platform/decisions/outcomes" />
              <WorkflowLink title="IntelligenceOS Learning" description="Turn observed evidence patterns into new reviewable recommendations." count={observed.length} href="/platform/intelligence/learning" />
              <WorkflowLink title="Decision History" description="Trace decisions, canonical revisions, implementations, and observations." href="/platform/decisions/history" />
            </div>
          </section>
          ...
          <section style={{ ...panelStyle, padding: 22 }}>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', marginTop: 16, paddingTop: 14, color: '#777B8D', fontSize: 11, lineHeight: 1.55 }}>
              Counts are derived from the current local Decision Engine store. Server-side persistence, identity, permissions, and multi-user review remain future infrastructure work.
            </div>
          </section>
        </div>
        ...
        <section style={{ ...panelStyle, padding: 22, marginTop: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <h2 style={{ margin: 0, fontSize: 20 }}>Recently changed recommendations</h2>
            <a href="/platform/decisions/history" style={{ color: '#DABFFF', fontSize: 12, textDecoration: 'none' }}>Open full history →</a>
          </div>

          {recent.length === 0 ? (
            <p style={{ color: '#777B8D', margin: 0 }}>No recommendations recorded yet.</p>
          ) : (
            <div style={{ display: 'grid', gap: 9 }}>
              {recent.map((item) => (
                <article key={item.id} style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.12)', padding: 13, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                  <div>
                    <div style={{ color: '#FFBFDE', fontSize: 10, fontWeight: 800, letterSpacing: '0.07em', textTransform: 'uppercase' }}>{item.origin} → {item.targetOS}</div>
                    <div style={{ fontWeight: 700, marginTop: 5 }}>{item.title}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ color: '#DABFFF', fontSize: 11, fontWeight: 800 }}>{item.status}</div>
                    <div style={{ color: '#777B8D', fontSize: 10, marginTop: 4 }}>{new Date(item.updatedAt).toLocaleString()}</div>
                  </div>
                </article>
              ))}
            </div>
